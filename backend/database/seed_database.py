...
def seed_database(engine):
    ...
    with Session(engine) as session:
        ...
        link1 = UserConversationLink(user_id=users[0].id, conversation_id=conv1.id)
        link2 = UserConversationLink(user_id=users[1].id, conversation_id=conv1.id)
        session.add(link1)
        session.add(link2)
        ...
        link3 = UserConversationLink(user_id=users[0].id, conversation_id=conv2.id)
        link4 = UserConversationLink(user_id=users[2].id, conversation_id=conv2.id)
        link5 = UserConversationLink(user_id=users[3].id, conversation_id=conv2.id)
        session.add_all([link3, link4, link5])
        ...
        for user in users:
            link = UserConversationLink(user_id=user.id, conversation_id=conv3.id)
            session.add(link)
        ...
        for msg_data in messages_conv1:
            msg = Message(
                ...
                receiver_user_id=msg_data["receiver"].id,
                conversation_id=conv1.id,
                timestamp=datetime.now() - timedelta(minutes=msg_data["minutes_ago"])
            )
            session.add(msg)
        
        # Messages for Conversation 2 (Group: Alice, Charlie, Diana)
        messages_conv2 = [
            {"sender": users[0], "receiver": users[2], "content": "Hey everyone! Planning a Tokyo trip!", "minutes_ago": 100},
            {"sender": users[2], "receiver": users[0], "content": "Count me in! 🎌", "minutes_ago": 95},
            {"sender": users[3], "receiver": users[0], "content": "I'm so excited! When are we thinking?", "minutes_ago": 90},
            {"sender": users[0], "receiver": users[2], "content": "How about late spring? Cherry blossoms!", "minutes_ago": 85},
            {"sender": users[2], "receiver": users[0], "content": "Perfect! I'll start looking at flights", "minutes_ago": 80},
            {"sender": users[3], "receiver": users[0], "content": "I found an amazing ramen place we HAVE to try", "minutes_ago": 75},
            {"sender": users[0], "receiver": users[2], "content": "Add it to the trip! I'll vote yes 👍", "minutes_ago": 70},
        ]
        ...
        tokyo_events = [
            ...
            },
            {
                ...
                "location": {
                    "place_id": "ChIJISz8NjyLGGAR5JBaOCZa4RQ",
                    "name": "Tsukiji Outer Market",
                    "latitude": 35.6654,
                    "longitude": 139.7707,
            ...
            },
            {
                ...
                "location": {
                    "place_id": "ChIJCewJkL2LGGARHS_m2NRpJKY",
                    "name": "Tokyo Tower",
                    "latitude": 35.6586,
                    "longitude": 139.7454,
            ...
            },
            {
                ...
                "location": {
                    "place_id": "ChIJp6kFt9eMGGARJ1vvvHHiPIg",
                    "name": "Shibuya Crossing",
                    "latitude": 35.6595,
                    "longitude": 139.7004,
            ...
            },
            {
                ...
                "location": {
                    "place_id": "ChIJQSaJPauOGGARgPvT-4gEbYU",
                    "name": "teamLab Borderless",
                    "latitude": 35.6246,
                    "longitude": 139.7755,
        ...
        print("-" * 50)
        for user_data in users_data:
            print(f"Username: {user_data['username']:<10} | Email: {user_data['email']:<25} | Password: {user_data['password']}")
        print("="*50)
